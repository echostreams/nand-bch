...
/* Default values */
int df_m = 15;              // BCH code over GF(2**mm)
...
void print_hex(int length, int *Binary_data, FILE* std)
// Print the binary data in HEX form
// 1100 1010 = 5 3
{
    int i, j, l, v;
    l = (int)ceil((double)length / 4);
 ...
 */
{
    ...
    int* gen_roots = malloc((nn + 1) * sizeof(int));
    int* gen_roots_true = malloc((nn + 1) * sizeof(int));
    ...
    if (Verbose)
    {
        fprintf(stderr, "# The Generator Polynomial (degree=%d) is:\n", rr);
        for (i = 0; i <= rr; i++)
